...
Det er realistisk — og det er realistisk *netop fordi* fundamentet allerede er
...
— det er i *gengivelsen* og i *redigeringsværktøjerne*.
...
|---|---|---|
| **Vægge** som udfyldt poché med rene hjørnesamlinger | Vægge tegnes som *streger* (`<line>`, stroke = tykkelse) | Stor: kræver væg-poché som lukkede polygoner + hjørne-/T-samlingsløser |
| **Åbninger** klippet ind i væggen (dørsving-bue, vindue med karm/sprosser) | Åbninger tegnes som *cirkler* ovenpå væggen | Stor: kræver væg-gap (boolesk) + dør-/vindue-symboler med svingbue |
...
| **Målkæder** på alle sider (12.35 m, 3.84 m, indvendige mål) | Findes *kun* for beliggenhedsplan-polygon (`dimension-lines.ts`), ikke for plantegningen | Stor: dedikeret målkæde-motor (witness lines, kæde-niveauer, hjørnemål) |
...
`floor-plan-render-model.ts`: *"The renderer owns nothing"*). Det betyder at
...
### WS1 — Væg-geometri & poché *(stor, fundament for alt visuelt)*
...
### WS2 — Symbol- & inventarbibliotek *(mellem, højest wow pr. krone)*
...
### WS3 — Målkæde-motor *(stor, det "professionelle" signal)*
...
### WS4 — Lag, zoner & skravering *(mellem)*
...
### WS5 — Skabe- & redigeringsværktøjer *(mellem-stor, forudsætning for brugerplaner)*
...
### WS6 — Ark, titelblok & skala *(lille-mellem)*
...
### WS8 — Compliance-overlay & data-binding *(mellem, vores moat på tegningen)*
...
**Demo:** en *genereret* plan gengivet med rigtige vægge, døre og badeværelses-
...
3. **Beslutning om DXF/IFC** (WS7-sporet) — strategisk det vigtigste for at stå
   *ved siden af* Revit i stedet for under det. Anbefales prioriteret.
