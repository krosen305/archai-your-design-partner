...
export function taskFail(
  ...
  const shouldRetry =
    policy.retryOn.includes(failure.type) &&
    task.retryCount < policy.maxAttempts - 1
...
function printSession(manifest: SessionManifest): void {
  ...
  for (const task of manifest.tasks) {
    ...
    console.log(
      `  ${icon} [${task.agent.padEnd(11)}] ${task.id}${retry}  ${dur}`
    )
...
switch (cmd) {
  ...
  case 'view':
    if (!flags.session) { console.error('--session påkrævet'); process.exit(1) }
