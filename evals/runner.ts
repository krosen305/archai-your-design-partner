  ...
  switch (evalCase.scoring) {
    ...
    case 'structural':
      scoreResult = scoreStructural(
        output,
        evalCase.expected as Record<string, 'string' | 'number' | 'boolean' | 'array' | 'object' | 'defined'>
      )
      break
    case 'semantic':
...
async function main() {
  ...
  const avgScore = active.length > 0
    ? active.reduce((sum, r) => sum + r.score, 0) / active.length
    : 0

  console.log(`\n${'═'.repeat(60)}`)
  console.log(`Resultat: ${passed.length}/${active.length} bestået  |  Avg score: ${(avgScore * 100).toFixed(0)}%  |  Sprunget over: ${skipped.length}`)
