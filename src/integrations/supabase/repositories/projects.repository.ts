...
export async function loadProject(
  ...
  let query = supabaseAdmin
    .from("projects")
    .select(
      "id, address_full, address_kommune, address_kommunekode, address_matrikel, address_bbr, address_adresseid, address_postnr, address_postnrnavn, address_koordinater, address_ejerlavskode, address_matrikelnummer, compliance_data, compliance_done, current_step, project_data_status, heritage_save_value, is_fredet, grundareal_m2, bebygget_areal_m2, hard_stop, hard_stop_reason, budget_estimate, bfe_nr, billedanalyse, updated_at, tagform, taghaldning_grad, har_jordvarme, har_kaelder, kaelder_gulv_kote_m",
    )
    .eq("user_id", userId);
