// Generated from BBRKodelister.csv. Do not edit manually.
/* eslint-disable */
export const BBR_CODE_REGISTRY = {
  Afloebsforhold: {
    "1": {
      title: "Fælleskloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "10": {
      title: "Afløb til offentligt kloaksystem",
      disabled: true,
    },
    "101": {
      title: "SOP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "102": {
      title: "SOP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "103": {
      title: "SOP: Minirenseanlæg med nedsivning i faskine",
      disabled: false,
    },
    "104": {
      title: "SOP: Nedsivning til sivedræn",
      disabled: false,
    },
    "105": {
      title: "SOP: Samletank",
      disabled: false,
    },
    "106": {
      title: "SOP: Pileanlæg med nedsivning (uden membran)",
      disabled: false,
    },
    "107": {
      title: "SOP: Pileanlæg uden udledning (med membran)",
      disabled: false,
    },
    "108": {
      title: "SOP: Beplantede filteranlæg med nedsivning i faskine",
      disabled: false,
    },
    "109": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og direkte udledning",
      disabled: false,
    },
    "11": {
      title: "Afløb til fællesprivat kloaksystem",
      disabled: true,
    },
    "110": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og udledning til markdræn",
      disabled: false,
    },
    "12": {
      title:
        "Afløb til fællesprivat kloaksystem med tilslutning til spildevandsforsyningens kloaksystem",
      disabled: false,
    },
    "190": {
      title: "SOP: Andet",
      disabled: false,
    },
    "2": {
      title: "Fælleskloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "20": {
      title: "Afløb til samletank",
      disabled: true,
    },
    "201": {
      title: "SO: Biologisk sandfilter med direkte udledning",
      disabled: false,
    },
    "202": {
      title: "SO: Biologisk sandfilter med udledning til markdræn",
      disabled: false,
    },
    "203": {
      title: "SO: Minirensanlæg med direkte udledning",
      disabled: false,
    },
    "204": {
      title: "SO: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "205": {
      title: "SO: Beplantede filteranlæg med direkte udledning",
      disabled: false,
    },
    "206": {
      title: "SO: Beplantede filteranlæg med udledning til markdræn",
      disabled: false,
    },
    "21": {
      title: "Afløb til samletank for toiletvand og mekanisk rensning af øvrigt spildevand",
      disabled: false,
    },
    "29": {
      title: "Mekanisk rensning med nedsivningsanlæg med tilladelse",
      disabled: true,
    },
    "290": {
      title: "SO: Andet",
      disabled: false,
    },
    "3": {
      title: "Fælleskloakeret: spildevand",
      disabled: false,
    },
    "30": {
      title: "Mekanisk rensning med nedsivningsanlæg (tilladelse ikke påkrævet)",
      disabled: true,
    },
    "301": {
      title: "OP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "302": {
      title: "OP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "31": {
      title: "Mekanisk rensning med privat udledning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "32": {
      title: "Mekanisk og biologisk rensning (ældre anlæg uden renseklasse)",
      disabled: true,
    },
    "390": {
      title: "OP: Andet",
      disabled: false,
    },
    "4": {
      title: "Fælleskloakeret: tag- og overfladevand",
      disabled: false,
    },
    "401": {
      title: "O: Rodzoneanlæg med direkte udledning",
      disabled: false,
    },
    "402": {
      title: "O: Rodzoneanlæg med udledning til markdræn",
      disabled: false,
    },
    "403": {
      title: "O: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "404": {
      title: "O: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "490": {
      title: "O: Andet",
      disabled: false,
    },
    "5": {
      title: "Separatkloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "501": {
      title: "Øvrige renseløsninger: Mekanisk med direkte udledning",
      disabled: false,
    },
    "502": {
      title: "Øvrige renseløsninger: Mekanisk med udledning til markdræn",
      disabled: false,
    },
    "503": {
      title: "Øvrige renseløsninger: Ældre nedsivningsanlæg med nedsivning til sivebrønd",
      disabled: false,
    },
    "504": {
      title: "Udledning til jordoverfladen",
      disabled: false,
    },
    "505": {
      title: "Udledning urenset",
      disabled: false,
    },
    "590": {
      title: "Øvrige renseløsninger: Andet",
      disabled: false,
    },
    "6": {
      title: "Separatkloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "601": {
      title: "Anden type afløb (større end 30 PE med egen udledning)",
      disabled: false,
    },
    "7": {
      title: "Separatkloakeret: spildevand",
      disabled: false,
    },
    "70": {
      title: "Udledning uden rensning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "701": {
      title: "Intet afløb",
      disabled: false,
    },
    "75": {
      title: "Afløbsforhold er registreret på bygninger",
      disabled: false,
    },
    "8": {
      title: "Separatkloakeret: tag- og overfladevand",
      disabled: false,
    },
    "80": {
      title: "Anden type afløb",
      disabled: true,
    },
    "9": {
      title: "Spildevandskloakeret: Spildevand",
      disabled: false,
    },
    "90": {
      title: "Ingen udledning",
      disabled: true,
    },
  },
  AfvigendeEtager: {
    "0": {
      title: "Bygningen har ikke afvigende etager",
      disabled: false,
    },
    "10": {
      title: "Bygningen har afvigende etager",
      disabled: false,
    },
    "11": {
      title: "Bygningen indeholder hems",
      disabled: false,
    },
    "12": {
      title: "Bygningen indeholder etagegennembrydende rum",
      disabled: false,
    },
    "13": {
      title: "Bygningen indeholder indskudt etage",
      disabled: false,
    },
  },
  AsbestholdigtMateriale: {
    "1": {
      title: "Asbestholdigt ydervægsmateriale",
      disabled: false,
    },
    "2": {
      title: "Asbestholdigt tagdækningsmateriale",
      disabled: false,
    },
    "3": {
      title: "Asbestholdigt ydervægs- og tagdækningsmateriale",
      disabled: false,
    },
    "4": {
      title: "Øvrige asbestholdige materialer",
      disabled: false,
    },
    "5": {
      title: "Ikke asbest i bygningen",
      disabled: false,
    },
  },
  Badeforhold: {
    C: {
      title: "Adgang til badeværelse",
      disabled: false,
    },
    D: {
      title: "Hverken badeværelse eller adgang til badeværelse",
      disabled: false,
    },
    V: {
      title: "Badeværelse i enheden",
      disabled: false,
    },
  },
  BBRMessageAarsagskode: {
    "1": {
      title: "Denne BBR-meddelelse er hentet via selvbetjening.",
      disabled: true,
    },
    "20": {
      title:
        "Denne BBR-meddelelse er udskrevet på grund af ændringer i ejeroplysningerne i Ejerfortegnelsen. BBR-meddelelsen sendes til alle ejere/administratorer af ejendommen.",
      disabled: true,
    },
    "30": {
      title: "Denne BBR-meddelelse er udskrevet på anmodning.",
      disabled: false,
    },
    "31": {
      title: "Denne BBR-andelsboligudskrift er udskrevet på anmodning.",
      disabled: true,
    },
    "40": {
      title: "Denne BBR-meddelelse er udskrevet på grund af afsluttet byggesag.",
      disabled: false,
    },
    "41": {
      title: "Denne BBR-meddelelse er udskrevet på grund af indflytning.",
      disabled: true,
    },
    "42": {
      title:
        "Denne BBR-meddelelse er udskrevet på grund af delvis eller midlertidig afsluttet byggesag.",
      disabled: true,
    },
    "45": {
      title: "Denne BBR-meddelelse er udskrevet på grund af ændring uden byggesag.",
      disabled: true,
    },
    "46": {
      title: "Denne BBR-meddelelse er udskrevet på grund af opdeling af lejligheder.",
      disabled: true,
    },
    "47": {
      title: "Denne BBR-meddelelse er udskrevet på grund af sammenlægning af lejligheder.",
      disabled: true,
    },
    "48": {
      title:
        "Denne BBR-meddelelse er udskrevet på grund af opdeling/ændret opdeling i ejerlejligheder.",
      disabled: true,
    },
    "50": {
      title:
        "Denne BBR-meddelelse er udskrevet på grund af, at kommunen har foretaget rettelser af registreringen i BBR.",
      disabled: false,
    },
    "51": {
      title:
        "Der er et byggeri på ejendommen som ikke er færdigmeldt. Kommunen har nu registreret byggeriet som fuldført. Er byggeriet ikke færdigt, skal du orientere kommunen.",
      disabled: true,
    },
    "70": {
      title: "Denne BBR-meddelelse er udskrevet på grund af matrikulære ændringer.",
      disabled: false,
    },
    "80": {
      title:
        "Denne BBR-meddelelse er udskrevet, fordi ejer selv eller andre har rettet, slettet eller tilføjet oplysninger. Kontrollér venligst at ændringerne er korrekte.",
      disabled: true,
    },
    "81": {
      title:
        "Denne BBR-meddelelse er udskrevet fordi skatteforvaltningen har rettet, slettet eller tilføjet oplysninger på ejendommen.",
      disabled: true,
    },
    "98": {
      title: "Denne BBR-meddelelse er udskrevet manuelt med brugerdefineret årsagskodetekst",
      disabled: false,
    },
  },
  Boligtype: {
    "1": {
      title: "Egentlig beboelseslejlighed med eget køkken",
      disabled: false,
    },
    "2": {
      title: "Blandet bolig og erhverv med eget køkken",
      disabled: false,
    },
    "3": {
      title: "Enkeltværelse uden eget køkken",
      disabled: false,
    },
    "4": {
      title: "Fællesbolig",
      disabled: false,
    },
    "5": {
      title: "Sommer- eller fritidsbolig",
      disabled: false,
    },
    E: {
      title: "Andet",
      disabled: false,
    },
  },
  BygAfloebsforhold: {
    "1": {
      title: "Fælleskloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "10": {
      title: "Afløb til offentligt kloaksystem",
      disabled: true,
    },
    "101": {
      title: "SOP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "102": {
      title: "SOP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "103": {
      title: "SOP: Minirenseanlæg med nedsivning i faskine",
      disabled: false,
    },
    "104": {
      title: "SOP: Nedsivning til sivedræn",
      disabled: false,
    },
    "105": {
      title: "SOP: Samletank",
      disabled: false,
    },
    "106": {
      title: "SOP: Pileanlæg med nedsivning (uden membran)",
      disabled: false,
    },
    "107": {
      title: "SOP: Pileanlæg uden udledning (med membran)",
      disabled: false,
    },
    "108": {
      title: "SOP: Beplantede filteranlæg med nedsivning i faskine",
      disabled: false,
    },
    "109": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og direkte udledning",
      disabled: false,
    },
    "11": {
      title: "Afløb til fællesprivat kloaksystem",
      disabled: true,
    },
    "110": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og udledning til markdræn",
      disabled: false,
    },
    "12": {
      title:
        "Afløb til fællesprivat kloaksystem med tilslutning til spildevandsforsyningens kloaksystem",
      disabled: false,
    },
    "190": {
      title: "SOP: Andet",
      disabled: false,
    },
    "2": {
      title: "Fælleskloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "20": {
      title: "Afløb til samletank",
      disabled: true,
    },
    "201": {
      title: "SO: Biologisk sandfilter med direkte udledning",
      disabled: false,
    },
    "202": {
      title: "SO: Biologisk sandfilter med udledning til markdræn",
      disabled: false,
    },
    "203": {
      title: "SO: Minirensanlæg med direkte udledning",
      disabled: false,
    },
    "204": {
      title: "SO: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "205": {
      title: "SO: Beplantede filteranlæg med direkte udledning",
      disabled: false,
    },
    "206": {
      title: "SO: Beplantede filteranlæg med udledning til markdræn",
      disabled: false,
    },
    "21": {
      title: "Afløb til samletank for toiletvand og mekanisk rensning af øvrigt spildevand",
      disabled: false,
    },
    "29": {
      title: "Mekanisk rensning med nedsivningsanlæg med tilladelse",
      disabled: true,
    },
    "290": {
      title: "SO: Andet",
      disabled: false,
    },
    "3": {
      title: "Fælleskloakeret: spildevand",
      disabled: false,
    },
    "30": {
      title: "Mekanisk rensning med nedsivningsanlæg (tilladelse ikke påkrævet)",
      disabled: true,
    },
    "301": {
      title: "OP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "302": {
      title: "OP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "31": {
      title: "Mekanisk rensning med privat udledning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "32": {
      title: "Mekanisk og biologisk rensning (ældre anlæg uden renseklasse)",
      disabled: true,
    },
    "390": {
      title: "OP: Andet",
      disabled: false,
    },
    "4": {
      title: "Fælleskloakeret: tag- og overfladevand",
      disabled: false,
    },
    "401": {
      title: "O: Rodzoneanlæg med direkte udledning",
      disabled: false,
    },
    "402": {
      title: "O: Rodzoneanlæg med udledning til markdræn",
      disabled: false,
    },
    "403": {
      title: "O: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "404": {
      title: "O: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "490": {
      title: "O: Andet",
      disabled: false,
    },
    "5": {
      title: "Separatkloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "501": {
      title: "Øvrige renseløsninger: Mekanisk med direkte udledning",
      disabled: false,
    },
    "502": {
      title: "Øvrige renseløsninger: Mekanisk med udledning til markdræn",
      disabled: false,
    },
    "503": {
      title: "Øvrige renseløsninger: Ældre nedsivningsanlæg med nedsivning til sivebrønd",
      disabled: false,
    },
    "504": {
      title: "Udledning til jordoverfladen",
      disabled: false,
    },
    "505": {
      title: "Udledning urenset",
      disabled: false,
    },
    "590": {
      title: "Øvrige renseløsninger: Andet",
      disabled: false,
    },
    "6": {
      title: "Separatkloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "601": {
      title: "Anden type afløb (større end 30 PE med egen udledning)",
      disabled: false,
    },
    "7": {
      title: "Separatkloakeret: spildevand",
      disabled: false,
    },
    "70": {
      title: "Udledning uden rensning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "701": {
      title: "Intet afløb",
      disabled: false,
    },
    "75": {
      title: "Afløbsforhold er registreret på bygninger",
      disabled: true,
    },
    "8": {
      title: "Separatkloakeret: tag- og overfladevand",
      disabled: false,
    },
    "80": {
      title: "Anden type afløb",
      disabled: true,
    },
    "9": {
      title: "Spildevandskloakeret: Spildevand",
      disabled: false,
    },
    "90": {
      title: "Ingen udledning",
      disabled: true,
    },
  },
  BygAnvendelse: {
    "110": {
      title: "Stuehus til landbrugsejendom",
      disabled: false,
    },
    "120": {
      title: "Fritliggende enfamiliehus",
      disabled: false,
    },
    "121": {
      title: "Sammenbygget enfamiliehus",
      disabled: false,
    },
    "122": {
      title: "Fritliggende enfamiliehus i tæt-lav bebyggelse",
      disabled: false,
    },
    "130": {
      title: "(UDFASES) Række-, kæde-, eller dobbelthus (lodret adskillelse mellem enhederne).",
      disabled: true,
    },
    "131": {
      title: "Række-, kæde- og klyngehus",
      disabled: false,
    },
    "132": {
      title: "Dobbelthus",
      disabled: false,
    },
    "140": {
      title: "Etagebolig-bygning, flerfamiliehus eller to-familiehus",
      disabled: false,
    },
    "150": {
      title: "Kollegium",
      disabled: false,
    },
    "160": {
      title: "Boligbygning til døgninstitution",
      disabled: false,
    },
    "185": {
      title: "Anneks i tilknytning til helårsbolig.",
      disabled: false,
    },
    "190": {
      title: "Anden bygning til helårsbeboelse",
      disabled: false,
    },
    "210": {
      title:
        "(UDFASES) Bygning til erhvervsmæssig produktion vedrørende landbrug, gartneri, råstofudvinding o. lign",
      disabled: true,
    },
    "211": {
      title: "Stald til svin",
      disabled: false,
    },
    "212": {
      title: "Stald til kvæg, får mv.",
      disabled: false,
    },
    "213": {
      title: "Stald til fjerkræ",
      disabled: false,
    },
    "214": {
      title: "Minkhal",
      disabled: false,
    },
    "215": {
      title: "Væksthus",
      disabled: false,
    },
    "216": {
      title: "Lade til foder, afgrøder mv.",
      disabled: false,
    },
    "217": {
      title: "Maskinhus,  garage mv.",
      disabled: false,
    },
    "218": {
      title: "Lade til halm, hø mv.",
      disabled: false,
    },
    "219": {
      title: "Anden bygning til landbrug mv.",
      disabled: false,
    },
    "220": {
      title:
        "(UDFASES) Bygning til erhvervsmæssig produktion vedrørende industri, håndværk m.v. (fabrik, værksted o.lign.)",
      disabled: true,
    },
    "221": {
      title: "Bygning til industri med integreret produktionsapparat",
      disabled: false,
    },
    "222": {
      title: "Bygning til industri uden integreret produktionsapparat",
      disabled: false,
    },
    "223": {
      title: "Værksted",
      disabled: false,
    },
    "229": {
      title: "Anden bygning til produktion",
      disabled: false,
    },
    "230": {
      title: "(UDFASES) El-, gas-, vand- eller varmeværk, forbrændingsanstalt m.v.",
      disabled: true,
    },
    "231": {
      title: "Bygning til energiproduktion",
      disabled: false,
    },
    "232": {
      title: "Bygning til energidistribution",
      disabled: false,
    },
    "233": {
      title: "Bygning til vandforsyning",
      disabled: false,
    },
    "234": {
      title: "Bygning til håndtering af affald og spildevand",
      disabled: false,
    },
    "239": {
      title: "Anden bygning til energiproduktion og forsyning",
      disabled: false,
    },
    "290": {
      title: "(UDFASES) Anden bygning til landbrug, industri etc.",
      disabled: true,
    },
    "310": {
      title:
        "(UDFASES) Transport- og garageanlæg (fragtmandshal, lufthavnsbygning, banegårdsbygning, parkeringshus). Garage med plads til et eller to køretøjer registreres med anvendelseskode 910",
      disabled: true,
    },
    "311": {
      title: "Bygning til jernbane- og busdrift",
      disabled: false,
    },
    "312": {
      title: "Bygning til luftfart",
      disabled: false,
    },
    "313": {
      title: "Bygning til parkering- og transportanlæg",
      disabled: false,
    },
    "314": {
      title: "Bygning til parkering af flere end to køretøjer i tilknytning til boliger",
      disabled: false,
    },
    "315": {
      title: "Havneanlæg",
      disabled: false,
    },
    "319": {
      title: "Andet transportanlæg",
      disabled: false,
    },
    "320": {
      title: "(UDFASES) Bygning til kontor, handel, lager, herunder offentlig administration",
      disabled: true,
    },
    "321": {
      title: "Bygning til kontor",
      disabled: false,
    },
    "322": {
      title: "Bygning til detailhandel",
      disabled: false,
    },
    "323": {
      title: "Bygning til lager",
      disabled: false,
    },
    "324": {
      title: "Butikscenter",
      disabled: false,
    },
    "325": {
      title: "Tankstation",
      disabled: false,
    },
    "329": {
      title: "Anden bygning til kontor, handel og lager",
      disabled: false,
    },
    "330": {
      title: "(UDFASES) Bygning til hotel, restaurant, vaskeri, frisør og anden servicevirksomhed",
      disabled: true,
    },
    "331": {
      title: "Hotel, kro eller konferencecenter med overnatning",
      disabled: false,
    },
    "332": {
      title: "Bed \u0026 breakfast mv.",
      disabled: false,
    },
    "333": {
      title: "Restaurant, café og konferencecenter uden overnatning",
      disabled: false,
    },
    "334": {
      title: "Privat servicevirksomhed som frisør, vaskeri, netcafé mv.",
      disabled: false,
    },
    "339": {
      title: "Anden bygning til serviceerhverv",
      disabled: false,
    },
    "390": {
      title: "(UDFASES) Anden bygning til transport, handel etc",
      disabled: true,
    },
    "410": {
      title:
        "(UDFASES) Bygning til biograf, teater, erhvervsmæssig udstilling, bibliotek, museum, kirke o. lign.",
      disabled: true,
    },
    "411": {
      title: "Biograf, teater, koncertsted mv.",
      disabled: false,
    },
    "412": {
      title: "Museum",
      disabled: false,
    },
    "413": {
      title: "Bibliotek",
      disabled: false,
    },
    "414": {
      title: "Kirke eller anden bygning til trosudøvelse for statsanerkendte trossamfund",
      disabled: false,
    },
    "415": {
      title: "Forsamlingshus",
      disabled: false,
    },
    "416": {
      title: "Forlystelsespark",
      disabled: false,
    },
    "419": {
      title: "Anden bygning til kulturelle formål",
      disabled: false,
    },
    "420": {
      title:
        "(UDFASES) Bygning til undervisning og forskning (skole, gymnasium, forskningslabratorium o.lign.).",
      disabled: true,
    },
    "421": {
      title: "Grundskole",
      disabled: false,
    },
    "422": {
      title: "Universitet",
      disabled: false,
    },
    "429": {
      title: "Anden bygning til undervisning og forskning",
      disabled: false,
    },
    "430": {
      title: "(UDFASES) Bygning til hospital, sygehjem, fødeklinik o. lign.",
      disabled: true,
    },
    "431": {
      title: "Hospital og sygehus",
      disabled: false,
    },
    "432": {
      title: "Hospice, behandlingshjem mv.",
      disabled: false,
    },
    "433": {
      title: "Sundhedscenter, lægehus, fødeklinik mv.",
      disabled: false,
    },
    "439": {
      title: "Anden bygning til sundhedsformål",
      disabled: false,
    },
    "440": {
      title: "(UDFASES) Bygning til daginstitution",
      disabled: true,
    },
    "441": {
      title: "Daginstitution",
      disabled: false,
    },
    "442": {
      title: "Servicefunktion på døgninstitution",
      disabled: false,
    },
    "443": {
      title: "Kaserne",
      disabled: false,
    },
    "444": {
      title: "Fængsel, arresthus mv.",
      disabled: false,
    },
    "449": {
      title: "Anden bygning til institutionsformål",
      disabled: false,
    },
    "451": {
      title: "Beskyttelsesrum",
      disabled: false,
    },
    "490": {
      title: "(UDFASES) Bygning til anden institution, herunder kaserne, fængsel o. lign.",
      disabled: true,
    },
    "510": {
      title: "Sommerhus",
      disabled: false,
    },
    "520": {
      title: "(UDFASES) Bygning til feriekoloni, vandrehjem o.lign. bortset fra sommerhus",
      disabled: true,
    },
    "521": {
      title: "Feriecenter, center til campingplads mv.",
      disabled: false,
    },
    "522": {
      title: "Bygning med ferielejligheder til erhvervsmæssig udlejning",
      disabled: false,
    },
    "523": {
      title: "Bygning med ferielejligheder til eget brug",
      disabled: false,
    },
    "529": {
      title: "Anden bygning til ferieformål",
      disabled: false,
    },
    "530": {
      title:
        "(UDFASES) Bygning i forbindelse med idrætsudøvelse (klubhus, idrætshal, svømmehal o. lign.)",
      disabled: true,
    },
    "531": {
      title: "Klubhus i forbindelse med fritid og idræt",
      disabled: false,
    },
    "532": {
      title: "Svømmehal",
      disabled: false,
    },
    "533": {
      title: "Idrætshal",
      disabled: false,
    },
    "534": {
      title: "Tribune i forbindelse med stadion",
      disabled: false,
    },
    "535": {
      title: "Bygning til træning og opstaldning af heste",
      disabled: false,
    },
    "539": {
      title: "Anden bygning til idrætformål",
      disabled: false,
    },
    "540": {
      title: "Kolonihavehus",
      disabled: false,
    },
    "585": {
      title: "Anneks i tilknytning til fritids- og sommerhus",
      disabled: false,
    },
    "590": {
      title: "Anden bygning til fritidsformål",
      disabled: false,
    },
    "910": {
      title: "Garage",
      disabled: false,
    },
    "920": {
      title: "Carport",
      disabled: false,
    },
    "930": {
      title: "Udhus",
      disabled: false,
    },
    "940": {
      title: "Drivhus",
      disabled: false,
    },
    "950": {
      title: "Fritliggende overdækning",
      disabled: false,
    },
    "960": {
      title: "Fritliggende udestue",
      disabled: false,
    },
    "970": {
      title: "Tiloversbleven landbrugsbygning",
      disabled: false,
    },
    "990": {
      title: "Faldefærdig bygning",
      disabled: false,
    },
    "999": {
      title: "Ukendt bygning",
      disabled: false,
    },
  },
  BygDaekningsafgift: {
    "0": {
      title: "Ikke omfattet af dækningsafgift",
      disabled: false,
    },
    "1": {
      title: "Omfattet af dækningsafgift",
      disabled: false,
    },
    "2": {
      title: "Delvist omfattet af dækningsafgift",
      disabled: false,
    },
  },
  Byggesagskode: {
    "1": {
      title: "BR - Tilladelsessag uden ibrugtagningstilladelse",
      disabled: false,
    },
    "2": {
      title: "(UDFASES) BR - Anmeldelsessag (garager, carporte, udhuse og nedrivning)",
      disabled: false,
    },
    "3": {
      title: "BR - Tilladelsessag med ibrugtagningstilladelse",
      disabled: false,
    },
    "4": {
      title: "BR - Tilladelsessag landbrugsbygning",
      disabled: false,
    },
    "5": {
      title: "(UDFASES) BR - Anmeldelsessag (øvrige)",
      disabled: false,
    },
    "6": {
      title: "BR – Tilladelsessag Nedrivning",
      disabled: false,
    },
    "7": {
      title: "BR – Lovliggørelse",
      disabled: false,
    },
  },
  Byggeskadeforsikringsselskab: {
    "0": {
      title: "Ingen byggeskadeforsikring",
      disabled: false,
    },
    "1": {
      title: "Tryg",
      disabled: false,
    },
    "10": {
      title: "Købstædernes Forsikring",
      disabled: false,
    },
    "100": {
      title: "Byggeskadeforsikring udløbet",
      disabled: false,
    },
    "11": {
      title: "ALKA",
      disabled: false,
    },
    "12": {
      title: "Frida Forsikring Agentur",
      disabled: false,
    },
    "13": {
      title: "NemForsikring",
      disabled: false,
    },
    "14": {
      title: "AXA",
      disabled: false,
    },
    "15": {
      title: "Husejernes Forsikring",
      disabled: false,
    },
    "16": {
      title: "Garbo",
      disabled: false,
    },
    "17": {
      title: "Marsh og McLennan Agency A/S",
      disabled: false,
    },
    "18": {
      title: "First Marine",
      disabled: false,
    },
    "19": {
      title: "Domus Forsikring A/S",
      disabled: false,
    },
    "2": {
      title: "Topdanmark",
      disabled: false,
    },
    "4": {
      title: "Codan",
      disabled: false,
    },
    "5": {
      title: "If Forsikring",
      disabled: false,
    },
    "6": {
      title: "Alm. Brand",
      disabled: false,
    },
    "7": {
      title: "Danske Forsikring",
      disabled: false,
    },
    "8": {
      title: "Caplloyd A/S",
      disabled: false,
    },
    "98": {
      title: "Dækket af byggeskadefonden",
      disabled: false,
    },
    "99": {
      title: "Ingen forsikring på grund af dispensation",
      disabled: false,
    },
  },
  BygherreForhold: {
    "10": {
      title: "Privatpersoner eller interessentskab",
      disabled: false,
    },
    "20": {
      title: "Alment boligselskab",
      disabled: false,
    },
    "30": {
      title: "Aktie-, anpart- eller andet selskab (undtagen interessent­skab)",
      disabled: false,
    },
    "40": {
      title: "Forening, legat eller selvejende institution",
      disabled: false,
    },
    "41": {
      title: "Privat andelsboligforening",
      disabled: false,
    },
    "50": {
      title: "Den kommune, hvori ejendommen er beliggende",
      disabled: false,
    },
    "60": {
      title: "Anden primærkommune",
      disabled: false,
    },
    "70": {
      title: "Region",
      disabled: false,
    },
    "80": {
      title: "Staten",
      disabled: false,
    },
    "90": {
      title:
        "Andet, herunder moderejendomme for bebyggelser, der er op­delt i ejerlejligheder samt ejendomme, der ejes af flere ka­te­gorier af ejere",
      disabled: false,
    },
  },
  BygSupplerendeVarme: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Varmepumpe",
      disabled: false,
    },
    "10": {
      title: "Biogasanlæg",
      disabled: false,
    },
    "2": {
      title: "Brændeovne og lignende med skorsten",
      disabled: false,
    },
    "3": {
      title: "Biopejse og lignende uden skorsten",
      disabled: false,
    },
    "4": {
      title: "Solvarmeanlæg",
      disabled: false,
    },
    "5": {
      title: "Pejs",
      disabled: false,
    },
    "6": {
      title: "Gasradiator",
      disabled: false,
    },
    "7": {
      title: "Elvarme",
      disabled: false,
    },
    "80": {
      title: "Andet",
      disabled: false,
    },
    "90": {
      title: "(UDFASES) Bygningen har ingen supplerende varme",
      disabled: true,
    },
  },
  BygVandforsyning: {
    "1": {
      title: "Alment vandforsyningsanlæg",
      disabled: false,
    },
    "2": {
      title: "Privat vandforsyningsanlæg",
      disabled: false,
    },
    "3": {
      title: "Enkeltindvindingsanlæg",
      disabled: false,
    },
    "4": {
      title: "Brønd",
      disabled: false,
    },
    "6": {
      title: "Ikke alment vandforsyningsanlæg",
      disabled: false,
    },
    "7": {
      title: "Vandforsyning er registreret på bygninger",
      disabled: true,
    },
    "9": {
      title: "Ingen vandforsyning",
      disabled: false,
    },
  },
  DispensationFritagelseIftKollektivVarmeforsyning: {
    "1": {
      title: "Dispensation er tidsbegrænset",
      disabled: false,
    },
    "2": {
      title: "Dispensationen er ikke tidsbegrænset",
      disabled: false,
    },
  },
  Driftstatus: {
    "1": {
      title: "I drift",
      disabled: false,
    },
    "2": {
      title: "Ikke i drift",
      disabled: false,
    },
  },
  Ejendomstype: {
    "1": {
      title: "Matrikuleret Areal",
      disabled: false,
    },
    "2": {
      title: "BPFG",
      disabled: false,
    },
    "3": {
      title: "Ejerlejlighed",
      disabled: false,
    },
  },
  Ejerforholdskode: {
    "10": {
      title: "Privatpersoner eller interessentskab",
      disabled: false,
    },
    "20": {
      title: "Alment boligselskab",
      disabled: false,
    },
    "30": {
      title: "Aktie-, anpart- eller andet selskab (undtagen interessent­skab)",
      disabled: false,
    },
    "40": {
      title: "Forening, legat eller selvejende institution",
      disabled: false,
    },
    "41": {
      title: "Privat andelsboligforening",
      disabled: false,
    },
    "50": {
      title: "Den kommune, hvori ejendommen er beliggende",
      disabled: false,
    },
    "60": {
      title: "Anden primærkommune",
      disabled: false,
    },
    "70": {
      title: "Region",
      disabled: false,
    },
    "80": {
      title: "Staten",
      disabled: false,
    },
    "90": {
      title:
        "Andet, herunder moderejendomme for bebyggelser, der er opdelt i ejerlejligheder samt ejendomme, der ejes af flere kategorier af ejere",
      disabled: false,
    },
    "99": {
      title: "Ikke beregnet",
      disabled: false,
    },
  },
  Elevator: {
    "0": {
      title: "Der er ikke elevator i opgangen",
      disabled: false,
    },
    "1": {
      title: "Der er elevator i opgangen",
      disabled: false,
    },
  },
  Energiforsyning: {
    "1": {
      title: "Gas fra værk",
      disabled: false,
    },
    "2": {
      title: "230 V el fra værk",
      disabled: false,
    },
    "3": {
      title: "400 V el fra værk",
      disabled: false,
    },
    "4": {
      title: "Både 230 V el og gas fra værk",
      disabled: false,
    },
    "5": {
      title: "Både 400 V el og gas fra værk",
      disabled: false,
    },
    "6": {
      title: "Hverken el eller gas fra værk",
      disabled: false,
    },
  },
  EnhAnvendelse: {
    "110": {
      title: "Stuehus til landbrugsejendom",
      disabled: false,
    },
    "120": {
      title: "Fritliggende enfamiliehus",
      disabled: false,
    },
    "121": {
      title: "Sammenbygget enfamiliehus",
      disabled: false,
    },
    "122": {
      title: "Fritliggende enfamiliehus i tæt-lav bebyggelse",
      disabled: false,
    },
    "130": {
      title: "(UDFASES) Række-, kæde- eller dobbelthus (lodret adskillelse mellem enhederne).",
      disabled: true,
    },
    "131": {
      title: "Række-, kæde- og klyngehus",
      disabled: false,
    },
    "132": {
      title: "Dobbelthus",
      disabled: false,
    },
    "140": {
      title: "Bolig i etageejendom, flerfamiliehus eller to-familiehus",
      disabled: false,
    },
    "150": {
      title: "Kollegiebolig",
      disabled: false,
    },
    "160": {
      title: "Bolig i døgninstitution",
      disabled: false,
    },
    "185": {
      title: "Anneks i tilknytning til helårsbolig",
      disabled: false,
    },
    "190": {
      title: "Anden enhed til helårsbeboelse",
      disabled: false,
    },
    "210": {
      title:
        "(UDFASES) Erhvervsmæssig produktion vedrørende landbrug, skovbrug, gartneri, råstofudvinding og lign.",
      disabled: true,
    },
    "211": {
      title: "Stald til svin",
      disabled: false,
    },
    "212": {
      title: "Stald til kvæg, får mv.",
      disabled: false,
    },
    "213": {
      title: "Stald til fjerkræ",
      disabled: false,
    },
    "214": {
      title: "Minkhal",
      disabled: false,
    },
    "215": {
      title: "Væksthus",
      disabled: false,
    },
    "216": {
      title: "Lade til foder, afgrøder mv.",
      disabled: false,
    },
    "217": {
      title: "Maskinhus,  garage mv.",
      disabled: false,
    },
    "218": {
      title: "Lade til halm, hø mv.",
      disabled: false,
    },
    "219": {
      title: "Anden enhed til landbrug mv.",
      disabled: false,
    },
    "220": {
      title:
        "(UDFASES) Erhvervsmæssig produktion vedrørende industri, håndværk m.v. (fabrik, værksted o. lign.)",
      disabled: true,
    },
    "221": {
      title: "Enhed til industri med integreret produktionsapparat",
      disabled: false,
    },
    "222": {
      title: "Enhed til industri uden integreret produktionsapparat",
      disabled: false,
    },
    "223": {
      title: "Værksted",
      disabled: false,
    },
    "229": {
      title: "Anden enhed til produktion",
      disabled: false,
    },
    "230": {
      title: "(UDFASES) El-, gas-, vand- eller varmeværk, forbrændingsanstalt o. lign.",
      disabled: true,
    },
    "231": {
      title: "Enhed til energiproduktion",
      disabled: false,
    },
    "232": {
      title: "Enhed til energidistribution",
      disabled: false,
    },
    "233": {
      title: "Enhed til vandforsyning",
      disabled: false,
    },
    "234": {
      title: "Enhed til håndtering af affald og spildevand",
      disabled: false,
    },
    "239": {
      title: "Anden enhed til energiproduktion og forsyning",
      disabled: false,
    },
    "290": {
      title:
        "(UDFASES) Anden enhed til produktion og lager i forbindelse med landbrug, industri o. lign.",
      disabled: true,
    },
    "310": {
      title:
        "(UDFASES) Transport- og garageanlæg (fragtmandshal, lufthavnsbygning,banegårdsbygning o. lign.)",
      disabled: true,
    },
    "311": {
      title: "Enhed til jernbane- og busdrift",
      disabled: false,
    },
    "312": {
      title: "Enhed til luftfart",
      disabled: false,
    },
    "313": {
      title: "Enhed til parkerings- og transportanlæg",
      disabled: false,
    },
    "314": {
      title: "Enhed til parkering af flere end to køretøjer i tilknytning til boliger",
      disabled: false,
    },
    "315": {
      title: "Havneanlæg",
      disabled: false,
    },
    "319": {
      title: "Andet transportanlæg",
      disabled: false,
    },
    "320": {
      title: "(UDFASES) Engroshandel og lager.",
      disabled: true,
    },
    "321": {
      title: "Enhed til kontor",
      disabled: false,
    },
    "322": {
      title: "Enhed til detailhandel",
      disabled: false,
    },
    "323": {
      title: "Enhed til lager",
      disabled: false,
    },
    "324": {
      title: "Butikscenter",
      disabled: false,
    },
    "325": {
      title: "Tankstation",
      disabled: false,
    },
    "329": {
      title: "Anden enhed til kontor, handel og lager",
      disabled: false,
    },
    "330": {
      title: "(UDFASES) Detailhandel m.v.",
      disabled: true,
    },
    "331": {
      title: "Hotel, kro eller konferencecenter med overnatning",
      disabled: false,
    },
    "332": {
      title: "Bed \u0026 breakfast mv.",
      disabled: false,
    },
    "333": {
      title: "Restaurant, café og konferencecenter uden overnatning",
      disabled: false,
    },
    "334": {
      title: "Privat servicevirksomhed som frisør, vaskeri, netcafé mv.",
      disabled: false,
    },
    "339": {
      title: "Anden enhed til serviceerhverv",
      disabled: false,
    },
    "340": {
      title: "(UDFASES) Pengeinstitut, forsikringsvirksomhed m.v.",
      disabled: true,
    },
    "350": {
      title:
        "(UDFASES) Kontor og liberale erhverv bortset fra offentlig administration  (kontorer for advokater, rådgivende ingeniører, klinikker o.lign.)",
      disabled: true,
    },
    "360": {
      title: "(UDFASES) Offentlig administration.",
      disabled: true,
    },
    "370": {
      title: "(UDFASES) Hotel, restauration, vaskeri, frisør og anden servicevirksomhed.",
      disabled: true,
    },
    "390": {
      title: "(UDFASES) Anden enhed til handel, transport etc.",
      disabled: true,
    },
    "410": {
      title: "(UDFASES) Biograf, teater, erhvervsmæssig udstilling m.v.",
      disabled: true,
    },
    "411": {
      title: "Biograf, teater, koncertsted mv.",
      disabled: false,
    },
    "412": {
      title: "Museum",
      disabled: false,
    },
    "413": {
      title: "Bibliotek",
      disabled: false,
    },
    "414": {
      title: "Kirke eller anden enhed til trosudøvelse for statsanerkendte trossamfund",
      disabled: false,
    },
    "415": {
      title: "Forsamlingshus",
      disabled: false,
    },
    "416": {
      title: "Forlystelsespark",
      disabled: false,
    },
    "419": {
      title: "Anden enhed til kulturelle formål",
      disabled: false,
    },
    "420": {
      title: "(UDFASES) Bibliotek, museum, kirke o. lign.",
      disabled: true,
    },
    "421": {
      title: "Grundskole",
      disabled: false,
    },
    "422": {
      title: "Universitet",
      disabled: false,
    },
    "429": {
      title: "Anden enhed til undervisning og forskning",
      disabled: false,
    },
    "430": {
      title: "(UDFASES) Undervisning og forskning (skole, gymnasium, forskningslaboratorium).",
      disabled: true,
    },
    "431": {
      title: "Hospital og sygehus",
      disabled: false,
    },
    "432": {
      title: "Hospice, behandlingshjem mv.",
      disabled: false,
    },
    "433": {
      title: "Sundhedscenter, lægehus, fødeklinik mv.",
      disabled: false,
    },
    "439": {
      title: "Anden enhed til sundhedsformål",
      disabled: false,
    },
    "440": {
      title: "(UDFASES) Hospital, fødeklinik o. lign.",
      disabled: true,
    },
    "441": {
      title: "Daginstitution",
      disabled: false,
    },
    "442": {
      title: "Servicefunktion på døgninstitution",
      disabled: false,
    },
    "443": {
      title: "Kaserne",
      disabled: false,
    },
    "444": {
      title: "Fængsel, arresthus mv.",
      disabled: false,
    },
    "449": {
      title: "Anden enhed til institutionsformål",
      disabled: false,
    },
    "450": {
      title: "(UDFASES) Daginstitution.",
      disabled: true,
    },
    "451": {
      title: "Enhed til beskyttelsesrum",
      disabled: false,
    },
    "490": {
      title: "(UDFASES) Anden institution, herunder kaserne, fængsel m.v.",
      disabled: true,
    },
    "510": {
      title: "Sommerhus.",
      disabled: false,
    },
    "520": {
      title: "(UDFASES) Enhed til feriekoloni, vandrehjem o.lign. bortset fra sommerhus",
      disabled: true,
    },
    "521": {
      title: "Feriecenter, center til campingplads mv.",
      disabled: false,
    },
    "522": {
      title: "Ferielejlighed til erhvervsmæssig udlejning",
      disabled: false,
    },
    "523": {
      title: "Ferielejlighed til eget brug",
      disabled: false,
    },
    "529": {
      title: "Anden enhed til ferieformål",
      disabled: false,
    },
    "530": {
      title:
        "(UDFASES) Enhed i forbindelse med idrætsudøvelse (klubhus, idrætshal, svømmehal o. lign.).",
      disabled: true,
    },
    "531": {
      title: "Klubhus i forbindelse med fritid- og idræt",
      disabled: false,
    },
    "532": {
      title: "Svømmehal",
      disabled: false,
    },
    "533": {
      title: "Idrætshal",
      disabled: false,
    },
    "534": {
      title: "Tribune i forbindelse med stadion",
      disabled: false,
    },
    "535": {
      title: "Enhed til træning og opstaldning af heste",
      disabled: false,
    },
    "539": {
      title: "Anden enhed til idrætsformål",
      disabled: false,
    },
    "540": {
      title: "Kolonihavehus",
      disabled: false,
    },
    "585": {
      title: "Anneks i tilknytning til fritids- og sommerhus",
      disabled: false,
    },
    "590": {
      title: "Anden enhed til fritidsformål",
      disabled: false,
    },
  },
  EnhDaekningsafgift: {
    "0": {
      title: "Ikke omfattet af dækningsafgift",
      disabled: false,
    },
    "1": {
      title: "Omfattet af dækningsafgift",
      disabled: false,
    },
  },
  EnhSupplerendeVarme: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Varmepumpe",
      disabled: false,
    },
    "10": {
      title: "Biogasanlæg",
      disabled: false,
    },
    "2": {
      title: "Brændeovne og lignende med skorsten",
      disabled: false,
    },
    "3": {
      title: "Biopejse og lignende uden skorsten",
      disabled: false,
    },
    "4": {
      title: "Solvarmeanlæg",
      disabled: false,
    },
    "5": {
      title: "Pejs",
      disabled: false,
    },
    "6": {
      title: "Gasradiator",
      disabled: false,
    },
    "7": {
      title: "Elvarme",
      disabled: false,
    },
    "80": {
      title: "Andet",
      disabled: false,
    },
    "90": {
      title: "(UDFASES) Bygningen har ingen supplerende varme",
      disabled: true,
    },
  },
  EnhVarmeinstallation: {
    "1": {
      title: "Fjernvarme/blokvarme",
      disabled: false,
    },
    "2": {
      title: "Centralvarme med én fyringsenhed",
      disabled: false,
    },
    "3": {
      title: "Ovn til fast og flydende brændsel",
      disabled: false,
    },
    "5": {
      title: "Varmepumpe",
      disabled: false,
    },
    "6": {
      title: "Centralvarme med to fyringsenheder",
      disabled: false,
    },
    "7": {
      title: "Elvarme",
      disabled: false,
    },
    "8": {
      title: "Gasradiator",
      disabled: false,
    },
    "9": {
      title: "Ingen varmeinstallation",
      disabled: false,
    },
    "99": {
      title: "Varmeinstallation er registreret på enheder",
      disabled: true,
    },
  },
  EtageType: {
    "0": {
      title: "Regulær etage",
      disabled: false,
    },
    "1": {
      title: "Tagetage",
      disabled: false,
    },
    "2": {
      title: "Kælder",
      disabled: false,
    },
  },
  Fordelingsnoegle: {
    "1": {
      title: "Manuel fordeling",
      disabled: false,
    },
    "2": {
      title: "Ligelig fordeling",
      disabled: false,
    },
    "3": {
      title: "Institutions fordeling",
      disabled: false,
    },
  },
  ForretningsHaendelse: {
    BUH: {
      title: "Bygning uden Husnummer",
      disabled: false,
    },
    BYG: {
      title: "Bygning",
      disabled: false,
    },
    ENH: {
      title: "Enhed",
      disabled: false,
    },
    GRU: {
      title: "Grund",
      disabled: false,
    },
    SAG: {
      title: "BBR-sag",
      disabled: false,
    },
    TEK: {
      title: "Teknisk Anlæg",
      disabled: false,
    },
    TUH: {
      title: "Teknisk Anlæg uden Husnummer",
      disabled: false,
    },
  },
  ForretningsOmraade: {
    BBR: {
      title: "54.15.05.05",
      disabled: false,
    },
  },
  ForretningsProcess: {
    "0": {
      title: "Ikke angivet",
      disabled: false,
    },
    "1": {
      title: "Oprettet grundet nybyggeri",
      disabled: false,
    },
    "10": {
      title: "Anmeldelsessag",
      disabled: false,
    },
    "11": {
      title: "Tilladelsessag",
      disabled: false,
    },
    "12": {
      title: "Opdateret grundet ændring i grunddataregister: Matriklen",
      disabled: false,
    },
    "13": {
      title: "Opdateret grundet ændring i grunddataregister: DAR",
      disabled: false,
    },
    "14": {
      title: "Opdateret grundet ændring i grunddataregister: Ejerfortegnelsen",
      disabled: false,
    },
    "15": {
      title: "Opdateret grundet ændring i grunddataregister: Ejendomsbeliggenhedsregisteret",
      disabled: false,
    },
    "16": {
      title: "Automatisk lukning af anmeldelsessag",
      disabled: false,
    },
    "17": {
      title: "Flytning af underliggende elementer på matrikel (Matrikulær ændring)",
      disabled: false,
    },
    "18": {
      title: "Fordelingsareal af fordelingsareal",
      disabled: false,
    },
    "19": {
      title: "Opdateret grundet ændret Sikkerhedsklassificering",
      disabled: false,
    },
    "2": {
      title: "Opdateret grundet til/ombygning",
      disabled: false,
    },
    "20": {
      title: "Fremdatering af indflytning",
      disabled: false,
    },
    "21": {
      title: "Opdatering af indberetning",
      disabled: false,
    },
    "22": {
      title: "ESR Event Processering",
      disabled: false,
    },
    "23": {
      title: "AWS Event Processering",
      disabled: false,
    },
    "24": {
      title: "Indberetnings services",
      disabled: false,
    },
    "25": {
      title: "SKATServices",
      disabled: false,
    },
    "26": {
      title: "EnergiindberetningProcessering",
      disabled: false,
    },
    "27": {
      title: "EJDbATilknytningHusnummerService",
      disabled: false,
    },
    "28": {
      title: "BPFG Tilknyttet gennem Ajorføring hos MU",
      disabled: false,
    },
    "3": {
      title: "Opdateret grundet nedrivning",
      disabled: false,
    },
    "4": {
      title: "Fejlrettelse af faktiske fejlregistreringer og udeladelser",
      disabled: false,
    },
    "5": {
      title: "Faktisk udført ændring uden byggesagsbehandling",
      disabled: false,
    },
    "6": {
      title: "Opdeling af enheder",
      disabled: false,
    },
    "7": {
      title: "Sammenlægning af enheder",
      disabled: false,
    },
    "8": {
      title: "Opdateret som følge af digital indberetning fra borger mm.",
      disabled: false,
    },
    "9": {
      title: "Opdateret som følge af digital indberetning fra SKAT",
      disabled: false,
    },
  },
  Fredning: {
    "1": {
      title: "Bygningen fredet iht. bygningsfredningsloven",
      disabled: false,
    },
    "2": {
      title: "Som 1, men med tinglyste bevaringsbestemmelser jf. lovens §15",
      disabled: false,
    },
    "3": {
      title: "Tinglyst bevaringsdeklaration, men bygningen ikke fredet",
      disabled: false,
    },
    "4": {
      title: "På bygningens middelalderlige bygningsdele er der tinglyst fredningsbestemmelser",
      disabled: false,
    },
    "5": {
      title: "Bygningen indeholder middelalderlige bygningsdele",
      disabled: false,
    },
    "6": {
      title: "Bygningen og dens umiddelbare omgivelser fredet iht. bygningsfredningsloven",
      disabled: false,
    },
    "7": {
      title: "Som 6, men med tinglyst bevaringsdeklaration",
      disabled: false,
    },
    "8": {
      title: "Bygningen bevaringsværdig",
      disabled: false,
    },
    "9": {
      title: "Bygningen medtaget i registrant, bevaringsplan mm.",
      disabled: false,
    },
  },
  GodkendtTomBolig: {
    "0": {
      title: "Krav om persontilmelding",
      disabled: false,
    },
    "100": {
      title: "Bolig uden krav om persontilmelding",
      disabled: false,
    },
  },
  GruAfloebsforhold: {
    "1": {
      title: "Fælleskloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "10": {
      title: "Afløb til offentligt kloaksystem",
      disabled: true,
    },
    "101": {
      title: "SOP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "102": {
      title: "SOP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "103": {
      title: "SOP: Minirenseanlæg med nedsivning i faskine",
      disabled: false,
    },
    "104": {
      title: "SOP: Nedsivning til sivedræn",
      disabled: false,
    },
    "105": {
      title: "SOP: Samletank",
      disabled: false,
    },
    "106": {
      title: "SOP: Pileanlæg med nedsivning (uden membran)",
      disabled: false,
    },
    "107": {
      title: "SOP: Pileanlæg uden udledning (med membran)",
      disabled: false,
    },
    "108": {
      title: "SOP: Beplantede filteranlæg med nedsivning i faskine",
      disabled: false,
    },
    "109": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og direkte udledning",
      disabled: false,
    },
    "11": {
      title: "Afløb til fællesprivat kloaksystem",
      disabled: true,
    },
    "110": {
      title: "SOP: Sandfiltre med P-fældning i bundfældningstanken og udledning til markdræn",
      disabled: false,
    },
    "12": {
      title:
        "Afløb til fællesprivat kloaksystem med tilslutning til spildevandsforsyningens kloaksystem",
      disabled: false,
    },
    "190": {
      title: "SOP: Andet",
      disabled: false,
    },
    "2": {
      title: "Fælleskloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "20": {
      title: "Afløb til samletank",
      disabled: true,
    },
    "201": {
      title: "SO: Biologisk sandfilter med direkte udledning",
      disabled: false,
    },
    "202": {
      title: "SO: Biologisk sandfilter med udledning til markdræn",
      disabled: false,
    },
    "203": {
      title: "SO: Minirensanlæg med direkte udledning",
      disabled: false,
    },
    "204": {
      title: "SO: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "205": {
      title: "SO: Beplantede filteranlæg med direkte udledning",
      disabled: false,
    },
    "206": {
      title: "SO: Beplantede filteranlæg med udledning til markdræn",
      disabled: false,
    },
    "21": {
      title: "Afløb til samletank for toiletvand og mekanisk rensning af øvrigt spildevand",
      disabled: false,
    },
    "29": {
      title: "Mekanisk rensning med nedsivningsanlæg med tilladelse",
      disabled: true,
    },
    "290": {
      title: "SO: Andet",
      disabled: false,
    },
    "3": {
      title: "Fælleskloakeret: spildevand",
      disabled: false,
    },
    "30": {
      title: "Mekanisk rensning med nedsivningsanlæg (tilladelse ikke påkrævet)",
      disabled: true,
    },
    "301": {
      title: "OP: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "302": {
      title: "OP: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "31": {
      title: "Mekanisk rensning med privat udledning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "32": {
      title: "Mekanisk og biologisk rensning (ældre anlæg uden renseklasse)",
      disabled: true,
    },
    "390": {
      title: "OP: Andet",
      disabled: false,
    },
    "4": {
      title: "Fælleskloakeret: tag- og overfladevand",
      disabled: false,
    },
    "401": {
      title: "O: Rodzoneanlæg med direkte udledning",
      disabled: false,
    },
    "402": {
      title: "O: Rodzoneanlæg med udledning til markdræn",
      disabled: false,
    },
    "403": {
      title: "O: Minirenseanlæg med direkte udledning",
      disabled: false,
    },
    "404": {
      title: "O: Minirenseanlæg med udledning til markdræn",
      disabled: false,
    },
    "490": {
      title: "O: Andet",
      disabled: false,
    },
    "5": {
      title: "Separatkloakeret: spildevand + tag- og overfladevand",
      disabled: false,
    },
    "501": {
      title: "Øvrige renseløsninger: Mekanisk med direkte udledning",
      disabled: false,
    },
    "502": {
      title: "Øvrige renseløsninger: Mekanisk med udledning til markdræn",
      disabled: false,
    },
    "503": {
      title: "Øvrige renseløsninger: Ældre nedsivningsanlæg med nedsivning til sivebrønd",
      disabled: false,
    },
    "504": {
      title: "Udledning til jordoverfladen",
      disabled: false,
    },
    "505": {
      title: "Udledning urenset",
      disabled: false,
    },
    "590": {
      title: "Øvrige renseløsninger: Andet",
      disabled: false,
    },
    "6": {
      title: "Separatkloakeret: spildevand + delvis tag- og overfladevand",
      disabled: false,
    },
    "601": {
      title: "Anden type afløb (større end 30 PE med egen udledning)",
      disabled: false,
    },
    "7": {
      title: "Separatkloakeret: spildevand",
      disabled: false,
    },
    "70": {
      title: "Udledning uden rensning direkte til vandløb, søer eller havet",
      disabled: true,
    },
    "701": {
      title: "Intet afløb",
      disabled: false,
    },
    "75": {
      title: "Afløbsforhold er registreret på bygninger",
      disabled: false,
    },
    "8": {
      title: "Separatkloakeret: tag- og overfladevand",
      disabled: false,
    },
    "80": {
      title: "Anden type afløb",
      disabled: true,
    },
    "9": {
      title: "Spildevandskloakeret: Spildevand",
      disabled: false,
    },
    "90": {
      title: "Ingen udledning",
      disabled: true,
    },
  },
  GruVandforsyning: {
    "1": {
      title: "Alment vandforsyningsanlæg",
      disabled: false,
    },
    "2": {
      title: "Privat vandforsyningsanlæg",
      disabled: false,
    },
    "3": {
      title: "Enkeltindvindingsanlæg",
      disabled: false,
    },
    "4": {
      title: "Brønd",
      disabled: false,
    },
    "6": {
      title: "Ikke alment vandforsyningsanlæg",
      disabled: false,
    },
    "7": {
      title: "Vandforsyning er registreret på bygninger",
      disabled: false,
    },
    "9": {
      title: "Ingen vandforsyning",
      disabled: false,
    },
  },
  Gulvbelaegning: {
    "1": {
      title: "Beton",
      disabled: false,
    },
    "2": {
      title: "Andet",
      disabled: false,
    },
    "3": {
      title: "Ingen",
      disabled: false,
    },
  },
  Indhold: {
    "10": {
      title: "Mineralske olieprodukter",
      disabled: false,
    },
    "11": {
      title: "Fuelolie",
      disabled: false,
    },
    "12": {
      title: "Fyringsgasolie",
      disabled: false,
    },
    "13": {
      title: "Diesel",
      disabled: false,
    },
    "14": {
      title: "Benzin",
      disabled: false,
    },
    "20": {
      title: "Biobrændstoffer",
      disabled: false,
    },
    "30": {
      title: "Affaldsprodukter",
      disabled: false,
    },
    "31": {
      title: "Oliebaserede affaldsprodukter",
      disabled: false,
    },
    "40": {
      title: "Gylle",
      disabled: false,
    },
    "50": {
      title: "Ajle, ensilagesaft, mælkerumsvand eller møddingvand",
      disabled: false,
    },
    "60": {
      title:
        "Øvrige stoffer, produkter og materiale der kan forurene grundvand, jord og undergrund",
      disabled: false,
    },
    "70": {
      title: "Korn",
      disabled: false,
    },
    "99": {
      title: "Andet",
      disabled: false,
    },
  },
  KildeTilKoordinatsaet: {
    E: {
      title: "Ejer",
      disabled: false,
    },
    K: {
      title: "Kommune",
      disabled: false,
    },
    L: {
      title: "Landinspektør",
      disabled: false,
    },
    M: {
      title: "Maskinelt dannet eller anden kilde",
      disabled: false,
    },
  },
  KildeTilOplysninger: {
    "1": {
      title: "Oplyst af ejer eller dennes repræsentant",
      disabled: false,
    },
    "2": {
      title: "Oplyst af kommunen",
      disabled: false,
    },
    "3": {
      title: "Oplyst af andre",
      disabled: false,
    },
    "4": {
      title: "Maskinelt oprettet",
      disabled: false,
    },
    "5": {
      title: "Oplyst og kontrolleret af kommunen",
      disabled: false,
    },
  },
  Klassifikation: {
    "1110": {
      title: "Tank",
      disabled: false,
    },
    "1120": {
      title: "Silo",
      disabled: false,
    },
    "1130": {
      title: "Gasbeholder",
      disabled: false,
    },
    "1140": {
      title: "Affaldsbeholder",
      disabled: false,
    },
    "1150": {
      title: "Udskiller",
      disabled: false,
    },
    "1210": {
      title: "Vindmølle",
      disabled: false,
    },
    "1220": {
      title: "Slanger til jordvarme",
      disabled: false,
    },
    "1230": {
      title: "Solcelleanlæg",
      disabled: false,
    },
    "1235": {
      title: "Solvarmeanlæg",
      disabled: false,
    },
    "1240": {
      title: "Nødstrømsforsyningsanlæg",
      disabled: false,
    },
    "1250": {
      title: "Transformerstation",
      disabled: false,
    },
    "1255": {
      title: "Oliefyr",
      disabled: false,
    },
    "1260": {
      title: "Elskab",
      disabled: false,
    },
    "1265": {
      title: "Naturgasfyr",
      disabled: false,
    },
    "1270": {
      title: "(UDFASES) Andet energiproducerende eller - distribuerende anlæg",
      disabled: true,
    },
    "1275": {
      title: "Halmfyr",
      disabled: false,
    },
    "1280": {
      title: "Biogasanlæg",
      disabled: false,
    },
    "1285": {
      title: "Pillefyr",
      disabled: false,
    },
    "1290": {
      title: "Brændeovn",
      disabled: false,
    },
    "1295": {
      title: "Andet energianlæg",
      disabled: false,
    },
    "1310": {
      title: "Vandtårn",
      disabled: false,
    },
    "1320": {
      title: "Pumpestation",
      disabled: false,
    },
    "1330": {
      title: "Swimmingpool",
      disabled: false,
    },
    "1340": {
      title: "Privat rensningsanlæg",
      disabled: false,
    },
    "1350": {
      title: "Offentligt rensningsanlæg",
      disabled: false,
    },
    "1360": {
      title: "Regnvandsanlæg",
      disabled: false,
    },
    "1905": {
      title: "Legeplads",
      disabled: false,
    },
    "1910": {
      title: "Teknikhus",
      disabled: false,
    },
    "1915": {
      title: "Døgnautomat",
      disabled: false,
    },
    "1920": {
      title: "Køleanlæg",
      disabled: false,
    },
    "1925": {
      title: "Kunstværk",
      disabled: false,
    },
    "1930": {
      title: "Sirene og mast med sirene",
      disabled: false,
    },
    "1935": {
      title: "Skilt",
      disabled: false,
    },
    "1940": {
      title: "Antenne og mast med antenne",
      disabled: false,
    },
    "1945": {
      title: "Dambrug",
      disabled: false,
    },
    "1950": {
      title: "Møddingsanlæg",
      disabled: false,
    },
    "1955": {
      title: "Andet teknisk anlæg",
      disabled: false,
    },
    "1960": {
      title: "(UDFASES) Ensilageanlæg",
      disabled: true,
    },
    "1965": {
      title: "Planlager",
      disabled: false,
    },
    "1970": {
      title: "Fortidsminde, historisk ruin",
      disabled: false,
    },
  },
  Koekkenforhold: {
    E: {
      title: "Eget køkken med afløb",
      disabled: false,
    },
    F: {
      title: "Adgang til fælles køkken",
      disabled: false,
    },
    G: {
      title: "Fast kogeinstallation i værelse eller på gang",
      disabled: false,
    },
    H: {
      title: "Ingen fast kogeinstallation",
      disabled: false,
    },
  },
  Kommunekode: {
    "0101": {
      title: "Københavns Kommune",
      disabled: false,
    },
    "0147": {
      title: "Frederiksberg Kommune",
      disabled: false,
    },
    "0151": {
      title: "Ballerup Kommune",
      disabled: false,
    },
    "0153": {
      title: "Brøndby Kommune",
      disabled: false,
    },
    "0155": {
      title: "Dragør Kommune",
      disabled: false,
    },
    "0157": {
      title: "Gentofte Kommune",
      disabled: false,
    },
    "0159": {
      title: "Gladsaxe Kommune",
      disabled: false,
    },
    "0161": {
      title: "Glostrup Kommune",
      disabled: false,
    },
    "0163": {
      title: "Herlev Kommune",
      disabled: false,
    },
    "0165": {
      title: "Albertslund Kommune",
      disabled: false,
    },
    "0167": {
      title: "Hvidovre Kommune",
      disabled: false,
    },
    "0169": {
      title: "Høje Taastrup Kommune",
      disabled: false,
    },
    "0173": {
      title: "Lyngby-Taarbæk Kommune",
      disabled: false,
    },
    "0175": {
      title: "Rødovre Kommune",
      disabled: false,
    },
    "0183": {
      title: "Ishøj Kommune",
      disabled: false,
    },
    "0185": {
      title: "Tårnby Kommune",
      disabled: false,
    },
    "0187": {
      title: "Vallensbæk Kommune",
      disabled: false,
    },
    "0190": {
      title: "Furesø Kommune",
      disabled: false,
    },
    "0201": {
      title: "Allerød Kommune",
      disabled: false,
    },
    "0210": {
      title: "Fredensborg Kommune",
      disabled: false,
    },
    "0217": {
      title: "Helsingør Kommune",
      disabled: false,
    },
    "0219": {
      title: "Hillerød Kommune",
      disabled: false,
    },
    "0223": {
      title: "Hørsholm Kommune",
      disabled: false,
    },
    "0230": {
      title: "Rudersdal Kommune",
      disabled: false,
    },
    "0240": {
      title: "Egedal Kommune",
      disabled: false,
    },
    "0250": {
      title: "Frederikssund Kommune",
      disabled: false,
    },
    "0253": {
      title: "Greve Kommune",
      disabled: false,
    },
    "0259": {
      title: "Køge Kommune",
      disabled: false,
    },
    "0260": {
      title: "Halsnæs Kommune",
      disabled: false,
    },
    "0265": {
      title: "Roskilde Kommune",
      disabled: false,
    },
    "0269": {
      title: "Solrød Kommune",
      disabled: false,
    },
    "0270": {
      title: "Gribskov Kommune",
      disabled: false,
    },
    "0306": {
      title: "Odsherred Kommune",
      disabled: false,
    },
    "0316": {
      title: "Holbæk Kommune",
      disabled: false,
    },
    "0320": {
      title: "Faxe Kommune",
      disabled: false,
    },
    "0326": {
      title: "Kalundborg Kommune",
      disabled: false,
    },
    "0329": {
      title: "Ringsted Kommune",
      disabled: false,
    },
    "0330": {
      title: "Slagelse Kommune",
      disabled: false,
    },
    "0336": {
      title: "Stevns Kommune",
      disabled: false,
    },
    "0340": {
      title: "Sorø Kommune",
      disabled: false,
    },
    "0350": {
      title: "Lejre Kommune",
      disabled: false,
    },
    "0360": {
      title: "Lolland Kommune",
      disabled: false,
    },
    "0370": {
      title: "Næstved Kommune",
      disabled: false,
    },
    "0376": {
      title: "Guldborgsund Kommune",
      disabled: false,
    },
    "0390": {
      title: "Vordingborg Kommune",
      disabled: false,
    },
    "0400": {
      title: "Bornholms Regionskommune",
      disabled: false,
    },
    "0410": {
      title: "Middelfart Kommune",
      disabled: false,
    },
    "0420": {
      title: "Assens Kommune",
      disabled: false,
    },
    "0430": {
      title: "Faaborg-Midtfyn Kommune",
      disabled: false,
    },
    "0440": {
      title: "Kerteminde Kommune",
      disabled: false,
    },
    "0450": {
      title: "Nyborg Kommune",
      disabled: false,
    },
    "0461": {
      title: "Odense Kommune",
      disabled: false,
    },
    "0479": {
      title: "Svendborg Kommune",
      disabled: false,
    },
    "0480": {
      title: "Nordfyns Kommune",
      disabled: false,
    },
    "0482": {
      title: "Langeland Kommune",
      disabled: false,
    },
    "0492": {
      title: "Ærø Kommune",
      disabled: false,
    },
    "0510": {
      title: "Haderslev Kommune",
      disabled: false,
    },
    "0530": {
      title: "Billund Kommune",
      disabled: false,
    },
    "0540": {
      title: "Sønderborg Kommune",
      disabled: false,
    },
    "0550": {
      title: "Tønder Kommune",
      disabled: false,
    },
    "0561": {
      title: "Esbjerg Kommune",
      disabled: false,
    },
    "0563": {
      title: "Fanø Kommune",
      disabled: false,
    },
    "0573": {
      title: "Varde Kommune",
      disabled: false,
    },
    "0575": {
      title: "Vejen Kommune",
      disabled: false,
    },
    "0580": {
      title: "Aabenraa Kommune",
      disabled: false,
    },
    "0607": {
      title: "Fredericia Kommune",
      disabled: false,
    },
    "0615": {
      title: "Horsens Kommune",
      disabled: false,
    },
    "0621": {
      title: "Kolding Kommune",
      disabled: false,
    },
    "0630": {
      title: "Vejle Kommune",
      disabled: false,
    },
    "0657": {
      title: "Herning Kommune",
      disabled: false,
    },
    "0661": {
      title: "Holstebro Kommune",
      disabled: false,
    },
    "0665": {
      title: "Lemvig Kommune",
      disabled: false,
    },
    "0671": {
      title: "Struer Kommune",
      disabled: false,
    },
    "0706": {
      title: "Syddjurs Kommune",
      disabled: false,
    },
    "0707": {
      title: "Norddjurs Kommune",
      disabled: false,
    },
    "0710": {
      title: "Favrskov Kommune",
      disabled: false,
    },
    "0727": {
      title: "Odder Kommune",
      disabled: false,
    },
    "0730": {
      title: "Randers Kommune",
      disabled: false,
    },
    "0740": {
      title: "Silkeborg Kommune",
      disabled: false,
    },
    "0741": {
      title: "Samsø Kommune",
      disabled: false,
    },
    "0746": {
      title: "Skanderborg Kommune",
      disabled: false,
    },
    "0751": {
      title: "Aarhus Kommune",
      disabled: false,
    },
    "0756": {
      title: "Ikast-Brande Kommune",
      disabled: false,
    },
    "0760": {
      title: "Ringkøbing-Skjern Kommune",
      disabled: false,
    },
    "0766": {
      title: "Hedensted Kommune",
      disabled: false,
    },
    "0773": {
      title: "Morsø Kommune",
      disabled: false,
    },
    "0779": {
      title: "Skive Kommune",
      disabled: false,
    },
    "0787": {
      title: "Thisted Kommune",
      disabled: false,
    },
    "0791": {
      title: "Viborg Kommune",
      disabled: false,
    },
    "0810": {
      title: "Brønderslev Kommune",
      disabled: false,
    },
    "0813": {
      title: "Frederikshavn Kommune",
      disabled: false,
    },
    "0820": {
      title: "Vesthimmerlands Kommune",
      disabled: false,
    },
    "0825": {
      title: "Læsø Kommune",
      disabled: false,
    },
    "0840": {
      title: "Rebild Kommune",
      disabled: false,
    },
    "0846": {
      title: "Mariagerfjord Kommune",
      disabled: false,
    },
    "0849": {
      title: "Jammerbugt Kommune",
      disabled: false,
    },
    "0851": {
      title: "Aalborg Kommune",
      disabled: false,
    },
    "0860": {
      title: "Hjørring Kommune",
      disabled: false,
    },
  },
  KondemneretBoligenhed: {
    "0": {
      title: "Ikke kondemneret boligenhed",
      disabled: false,
    },
    "1": {
      title: "Kondemneret boligenhed",
      disabled: false,
    },
  },
  Koordinatsystem: {
    "1": {
      title: "(UDFASES )System 34",
      disabled: true,
    },
    "2": {
      title: "(UDFASES) System 45",
      disabled: true,
    },
    "3": {
      title: "(UDFASES) KP2000 (System 2000)",
      disabled: true,
    },
    "4": {
      title: "(UDFASES) UTM ED50",
      disabled: true,
    },
    "5": {
      title: "UTM 32 Euref89",
      disabled: false,
    },
  },
  KvalitetAfKoordinatsaet: {
    "1": {
      title: "Sikker geokodning",
      disabled: false,
    },
    "2": {
      title: "(UDFASES) Næsten sikker",
      disabled: false,
    },
    "3": {
      title: "Usikker geokodning",
      disabled: false,
    },
  },
  Livscyklus: {
    "1": {
      title: "Start",
      disabled: false,
    },
    "10": {
      title: "Historisk",
      disabled: false,
    },
    "11": {
      title: "Fejlregistreret",
      disabled: false,
    },
    "12": {
      title: "Midlertidig Afsluttet",
      disabled: false,
    },
    "13": {
      title: "Delvis Afsluttet",
      disabled: false,
    },
    "14": {
      title: "Henlagt",
      disabled: false,
    },
    "15": {
      title: "Modtaget",
      disabled: false,
    },
    "16": {
      title: "UnderBehandling",
      disabled: false,
    },
    "17": {
      title: "Fejl",
      disabled: false,
    },
    "18": {
      title: "Udført",
      disabled: false,
    },
    "19": {
      title: "Foreløbig",
      disabled: false,
    },
    "2": {
      title: "Projekteret",
      disabled: false,
    },
    "20": {
      title: "BomSagModtaget",
      disabled: false,
    },
    "21": {
      title: "BomSagUnderBehandling",
      disabled: false,
    },
    "22": {
      title: "BomSagFejl",
      disabled: false,
    },
    "23": {
      title: "BomSagUdført",
      disabled: false,
    },
    "3": {
      title: "Under Opførelse",
      disabled: false,
    },
    "4": {
      title: "Sagsgrund",
      disabled: false,
    },
    "5": {
      title: "Oprettet",
      disabled: false,
    },
    "6": {
      title: "Opført",
      disabled: false,
    },
    "7": {
      title: "Gældende",
      disabled: false,
    },
    "8": {
      title: "Godkendt",
      disabled: false,
    },
    "9": {
      title: "Afsluttet",
      disabled: false,
    },
  },
  LovligAnvendelse: {
    A: {
      title: "Bolig har bevaret helårsstatus efter områdets udlægning til sommerhusområde",
      disabled: false,
    },
    B: {
      title:
        "Tidsbegrænset dispensation til helårsbeboelse til ejer. Dispensation bortfalder ved ejerskifte",
      disabled: false,
    },
    C: {
      title: "Dispensation til helårsbeboelse til ejer. Dispensation bortfalder ved ejerskifte",
      disabled: false,
    },
    D: {
      title: "Personlig ret for pensionister til helårsbeboelse",
      disabled: false,
    },
    E: {
      title: "Dispensation til afvikling af ulovlig helårsbeboelse",
      disabled: false,
    },
    I: {
      title: "Ikke relevant for denne enhed",
      disabled: false,
    },
  },
  MedlemsskabAfSplidevandforsyning: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Ikke medlemskab af spildevandsforsyning",
      disabled: false,
    },
    "2": {
      title: "Medlemskab af spildevandsforsyning",
      disabled: false,
    },
  },
  NiveauType: {
    "1": {
      title: "Grund",
      disabled: false,
    },
    "2": {
      title: "Bygning",
      disabled: false,
    },
    "3": {
      title: "TekniskAnlaeg",
      disabled: false,
    },
    "4": {
      title: "Etage",
      disabled: false,
    },
    "5": {
      title: "Opgang",
      disabled: false,
    },
    "6": {
      title: "Enhed",
      disabled: false,
    },
  },
  OffentligStoette: {
    "0": {
      title: "Ingen offentlig støtte",
      disabled: false,
    },
    "10": {
      title: "Almen familiebolig",
      disabled: false,
    },
    "15": {
      title: "Støttet privat udlejningsbolig",
      disabled: false,
    },
    "20": {
      title: "Støttet privat andelsbolig",
      disabled: false,
    },
    "25": {
      title: "Almen ungdomsbolig",
      disabled: false,
    },
    "30": {
      title: "Støttet privat ungdomsbolig",
      disabled: false,
    },
    "40": {
      title: "Almen ældrebolig",
      disabled: false,
    },
    "42": {
      title: "Almen plejebolig",
      disabled: false,
    },
    "80": {
      title: "Serviceareal",
      disabled: false,
    },
  },
  OmfattetAfByggeskadeforsikring: {
    "0": {
      title: "Bygningen er ikke omfattet af byggeskadeforsikring",
      disabled: false,
    },
    "10": {
      title: "Bygningen er omfattet af byggeskadeforsikring",
      disabled: false,
    },
    "11": {
      title: "Bygningen er opført som selvbyg",
      disabled: false,
    },
    "12": {
      title: "Udlejningsejendom",
      disabled: false,
    },
  },
  Opvarmningsmiddel: {
    "1": {
      title: "Elektricitet",
      disabled: false,
    },
    "2": {
      title: "Gasværksgas",
      disabled: false,
    },
    "3": {
      title: "Flydende brændsel",
      disabled: false,
    },
    "4": {
      title: "Fast brændsel",
      disabled: false,
    },
    "6": {
      title: "Halm",
      disabled: false,
    },
    "7": {
      title: "Naturgas",
      disabled: false,
    },
    "9": {
      title: "Andet",
      disabled: false,
    },
  },
  Oversvoemmelsesselvrisiko: {
    "0": {
      title: "Ingen udbetalt erstatning fra Naturskaderådet",
      disabled: false,
    },
    "1": {
      title: "(UDFASES) Bygningens selvrisiko er forhøjet til trin 1",
      disabled: true,
    },
    "2": {
      title: "(UDFASES) Bygningens selvrisiko er forhøjet til trin 2",
      disabled: true,
    },
    "3": {
      title:
        "Naturskaderådet registrerer, når der er udbetalt erstatning som følge af stormflod, oversvømmelse fra vandløb og søer og tørke. Læs om naturskadeordningerne på naturskaderaadet.dk.",
      disabled: false,
    },
  },
  Placering: {
    "0": {
      title: "Ukendt",
      disabled: false,
    },
    "1": {
      title: "Nedgravet",
      disabled: false,
    },
    "2": {
      title: "Over terræn, udendørs",
      disabled: false,
    },
    "3": {
      title: "Indendørs",
      disabled: false,
    },
  },
  PaaSoeTerritorie: {
    "0": {
      title: "Ikke på søterritorie",
      disabled: false,
    },
    "1": {
      title: "På søterritorie",
      disabled: false,
    },
  },
  Rensningspaabud: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Rensning ok. Intet påbud",
      disabled: false,
    },
    "2": {
      title: "Rensning skal forbedres til SOP",
      disabled: false,
    },
    "3": {
      title: "Rensning skal forbedres til SO",
      disabled: false,
    },
    "4": {
      title: "Rensning skal forbedres til OP",
      disabled: false,
    },
    "5": {
      title: "Rensning skal forbedres til O",
      disabled: false,
    },
    "6": {
      title: "Skal tilsluttes spildevandsforsyningsselskab",
      disabled: false,
    },
    "7": {
      title: "Skal tilsluttes separatkloakering",
      disabled: false,
    },
  },
  Sagsniveau: {
    "1": {
      title: "Grund",
      disabled: false,
    },
    "2": {
      title: "Bygning",
      disabled: false,
    },
    "3": {
      title: "TekniskAnlaeg",
      disabled: false,
    },
    "4": {
      title: "Etage",
      disabled: false,
    },
    "5": {
      title: "Opgang",
      disabled: false,
    },
    "6": {
      title: "Enhed",
      disabled: false,
    },
  },
  Sagstype: {
    "0": {
      title: "Sag på grund",
      disabled: false,
    },
    "1": {
      title: "Nybyggeri",
      disabled: false,
    },
    "2": {
      title: "Til/ombygning",
      disabled: false,
    },
    "31": {
      title: "Nedrivning (delvis)",
      disabled: false,
    },
    "32": {
      title: "Nedrivning (hel)",
      disabled: false,
    },
  },
  Sloejfning: {
    "1": {
      title: "Tanken er afblændet",
      disabled: false,
    },
    "10": {
      title: "Jordvarmeslangerne er sløjfet/taget ud af drift",
      disabled: false,
    },
    "2": {
      title: "Tanken er tømt og afblændet",
      disabled: false,
    },
    "3": {
      title: "Tanken er tømt, afblændet og opfyldt",
      disabled: false,
    },
    "4": {
      title: "Tanken er tømt, afblændet og påfyldningsstuds samt udluftningsrør afmonteret",
      disabled: false,
    },
  },
  Stoerrelsesklasse: {
    "1": {
      title: "Under 6.000 l",
      disabled: false,
    },
    "2": {
      title: "6.000 l - 100.000 l",
      disabled: false,
    },
    "3": {
      title: "Over 100.000 l",
      disabled: false,
    },
  },
  SupplerendeIndvendigKorrosionsbeskyttelse: {
    "1": {
      title: "Glasfiberbelægning",
      disabled: false,
    },
    "2": {
      title: "Organisk belægning",
      disabled: false,
    },
    "3": {
      title: "Anoder",
      disabled: false,
    },
    "4": {
      title: "Zinkstøvmaling",
      disabled: false,
    },
  },
  SupplerendeOplysningerOmKoordinatsaet: {
    "11": {
      title: "Koordinatsæt ligger i bygningen/anlægget (over jorden)",
      disabled: false,
    },
    "12": {
      title: "Koordinatsæt ligger i bygningen/anlægget (under jorden)",
      disabled: false,
    },
    "21": {
      title: "(UDFASES) Koordinatsæt ligger i bygningen/anlægget (over jorden)",
      disabled: true,
    },
    "22": {
      title: "(UDFASES) Koordinatsæt ligger i bygningen/anlægget (under jorden)",
      disabled: true,
    },
    "31": {
      title: "Koordinatsæt ligger på matriklen",
      disabled: false,
    },
    "32": {
      title: "Ukendt",
      disabled: false,
    },
  },
  SupplerendeVarme: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Varmepumpe",
      disabled: false,
    },
    "10": {
      title: "Biogasanlæg",
      disabled: false,
    },
    "2": {
      title: "Brændeovne og lignende med skorsten",
      disabled: false,
    },
    "3": {
      title: "Biopejse og lignende uden skorsten",
      disabled: false,
    },
    "4": {
      title: "Solvarmeanlæg",
      disabled: false,
    },
    "5": {
      title: "Pejs",
      disabled: false,
    },
    "6": {
      title: "Gasradiator",
      disabled: false,
    },
    "7": {
      title: "Elvarme",
      disabled: false,
    },
    "80": {
      title: "Andet",
      disabled: false,
    },
    "90": {
      title: "(UDFASES) Bygningen har ingen supplerende varme",
      disabled: false,
    },
  },
  Tagdaekningsmateriale: {
    "1": {
      title: "Tagpap med lille hældning",
      disabled: false,
    },
    "10": {
      title: "Fibercement uden asbest",
      disabled: false,
    },
    "11": {
      title: "Plastmaterialer",
      disabled: false,
    },
    "12": {
      title: "Glas",
      disabled: false,
    },
    "2": {
      title: "Tagpap med stor hældning",
      disabled: false,
    },
    "20": {
      title: "Levende tage",
      disabled: false,
    },
    "3": {
      title: "Fibercement herunder asbest",
      disabled: false,
    },
    "4": {
      title: "Betontagsten",
      disabled: false,
    },
    "5": {
      title: "Tegl",
      disabled: false,
    },
    "6": {
      title: "Metal",
      disabled: false,
    },
    "7": {
      title: "Stråtag",
      disabled: false,
    },
    "80": {
      title: "(UDFASES) Ingen",
      disabled: true,
    },
    "90": {
      title: "Andet materiale",
      disabled: false,
    },
  },
  Tilladelsesart: {
    "1": {
      title: "Upersonlig tilladelse uden tidsbegrænsning",
      disabled: false,
    },
    "2": {
      title: "Personlig tilladelse uden tidsbegrænsning",
      disabled: false,
    },
    "3": {
      title: "Upersonlig tilladelse med tidsbegrænsing",
      disabled: false,
    },
    "4": {
      title: "Personlig tilladelse med tidsbegrænsing",
      disabled: false,
    },
  },
  TilladelseTilAlternativBortskaffelseEllerAfledning: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Tilladelse meddelt",
      disabled: false,
    },
    "2": {
      title: "Tilladelse bortfaldet",
      disabled: false,
    },
  },
  TilladelseTilUdtraeden: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Tilladelse meddelt",
      disabled: false,
    },
    "2": {
      title: "Tilladelse bortfaldet",
      disabled: false,
    },
  },
  Toiletforhold: {
    A: {
      title: "Vandskyllende toilet uden for enheden",
      disabled: false,
    },
    B: {
      title: "Intet vandskyllende toilet",
      disabled: false,
    },
    T: {
      title: "Vandskyllende toilet i enheden",
      disabled: false,
    },
  },
  Udledningstilladelse: {
    "0": {
      title: "Ikke oplyst",
      disabled: false,
    },
    "1": {
      title: "Udledningstilladelse mangler",
      disabled: false,
    },
    "2": {
      title: "Renseanlæg etableret før 1974, derfor ikke behov for tilladelse",
      disabled: false,
    },
    "3": {
      title: "Udledningstilladelse til enkeltprivat renseanlæg",
      disabled: false,
    },
    "4": {
      title: "Udledningstilladelse til fællesprivat renseanlæg",
      disabled: false,
    },
    "5": {
      title: "Der foreligger ingen kendt tilladelse",
      disabled: false,
    },
    "6": {
      title: "Der foreligger tilladelse",
      disabled: false,
    },
    "7": {
      title: "Tilladelsesforhold er oplyst på bygningsniveau",
      disabled: false,
    },
  },
  Udlejningsforhold: {
    "1": {
      title: "Udlejet",
      disabled: false,
    },
    "2": {
      title: "Benyttet af ejeren",
      disabled: false,
    },
    "3": {
      title: "Ikke benyttet",
      disabled: false,
    },
  },
  Vandforsyning: {
    "1": {
      title: "Alment vandforsyningsanlæg",
      disabled: false,
    },
    "2": {
      title: "Privat vandforsyningsanlæg",
      disabled: false,
    },
    "3": {
      title: "Enkeltindvindingsanlæg",
      disabled: false,
    },
    "4": {
      title: "Brønd",
      disabled: false,
    },
    "6": {
      title: "Ikke alment vandforsyningsanlæg",
      disabled: false,
    },
    "7": {
      title: "Vandforsyning er registreret på bygninger",
      disabled: false,
    },
    "9": {
      title: "Ingen vandforsyning",
      disabled: false,
    },
  },
  Varmeinstallation: {
    "1": {
      title: "Fjernvarme/blokvarme",
      disabled: false,
    },
    "2": {
      title: "Centralvarme med én fyringsenhed",
      disabled: false,
    },
    "3": {
      title: "Ovn til fast og flydende brændsel",
      disabled: false,
    },
    "5": {
      title: "Varmepumpe",
      disabled: false,
    },
    "6": {
      title: "Centralvarme med to fyringsenheder",
      disabled: false,
    },
    "7": {
      title: "Elvarme",
      disabled: false,
    },
    "8": {
      title: "Gasradiator",
      disabled: false,
    },
    "9": {
      title: "Ingen varmeinstallation",
      disabled: false,
    },
    "99": {
      title: "Varmeinstallation er registreret på enheder",
      disabled: false,
    },
  },
  YdervaeggenesMateriale: {
    "1": {
      title: "Mursten",
      disabled: false,
    },
    "10": {
      title: "Fibercement uden asbest",
      disabled: false,
    },
    "11": {
      title: "Plastmaterialer",
      disabled: false,
    },
    "12": {
      title: "Glas",
      disabled: false,
    },
    "2": {
      title: "Letbetonsten",
      disabled: false,
    },
    "3": {
      title: "Fibercement herunder asbest",
      disabled: false,
    },
    "4": {
      title: "Bindingsværk",
      disabled: false,
    },
    "5": {
      title: "Træ",
      disabled: false,
    },
    "6": {
      title: "Betonelementer",
      disabled: false,
    },
    "8": {
      title: "Metal",
      disabled: false,
    },
    "80": {
      title: "Ingen",
      disabled: false,
    },
    "90": {
      title: "Andet materiale",
      disabled: false,
    },
  },
} as const;
